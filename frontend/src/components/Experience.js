  ...
  useEffect(() => {
    const fetchExperience = async () => {
      try {
        ...
        // Leave experiences as-is (no blocking spinner)
      }
    };
        ...
        <div className="space-y-8">
          {experiences.map((exp) => {
            ...
            return (
              <div
                key={exp.id}
                className={`p-8 bg-gradient-to-br ${colors.bg} rounded-xl border ${colors.border} transition-all duration-300 hover:transform hover:scale-[1.02]`}
              >
                {/* Header */}
                <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-6">
                  <div className="flex items-start space-x-4 mb-4 lg:mb-0">
                    ...
                    <div>
                      <h3 className="text-xl lg:text-2xl font-semibold text-white mb-2">{exp.title}</h3>
                      <p className={`text-lg font-medium ${colors.companyText} mb-2`}>{exp.company}</p>
                ...
                <div className="mb-6">
                  ...
                  <ul className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {exp.metrics.map((metric, idx) => (
                      <li key={idx} className="text-gray-300 text-sm bg-white/5 rounded-lg border border-gray-800 p-3">
                        {metric}
                      </li>
                    ))}
                  </ul>
